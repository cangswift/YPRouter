...
static YPRouterManager *sharedRouter = nil;
...
- (UINavigationController *)navigationController{
    ...
    // 根控制器是其他类型的情况
        ...
        if (tabbar != nil) {
            ...
            @throw [NSException exceptionWithName:@"错误" reason:@"控制器中没有导航控制器" userInfo:nil];
        }
    }
}
...
isHiddenTabbarWhenPush:(BOOL)isHiddenTabbarWhenPush
  completion:(void(^)(void))completion{
    ...
    if (isExistClass) {
        ...
        // 调用push
        [self.navigationController yp_PushViewController:vc animated:animated isHiddenTabbarWhenPush:isHiddenTabbarWhenPush completion:completion];
    }
    return;
}
...
- (void)popViewControllerAnimated:(BOOL)animated
    ...
    if ([vc respondsToSelector:@selector(yp_receivePreviousControllerWithParamsDic:)]) {
        [vc yp_receivePreviousControllerWithParamsDic:paramsDic];
    }
    [self.navigationController yp_popViewController:animated completion:completion];
}
...
- (void)popToViewController:(NSString *)viewController
    ...
    [self.navigationController.viewControllers enumerateObjectsUsingBlock:^(__kindof UIViewController * _Nonnull vc, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if ([viewController isEqualToString:NSStringFromClass([vc class])]) {
            ...
            [self.navigationController yp_popToViewController:vc animated:animated completion:completion];
            return;
        }
...
- (void)popToRootViewControllerAnimated:(BOOL)animated
    ...
    if ([navRootVC respondsToSelector:@selector(yp_receivePreviousControllerWithParamsDic:)]) {
        [navRootVC yp_receivePreviousControllerWithParamsDic:paramsDic];
    }
    [self.navigationController yp_popToRootViewController:animated completion:completion];
}
...
- (void)present:(NSString *)viewController
    ...
    if (isExistClass) {
        ...
        [self.navigationController.topViewController yp_presentViewController:vc animated:animated completion:completion];
    }
}
